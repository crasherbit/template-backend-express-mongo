...
export const assertUserCanMakeOrder = (user, totalAmount) => {
  if (user.balance < totalAmount) {
    throw createHttpError.PaymentRequired("User balance is insufficient to complete the order");
  }
};
...
export const assertValidStatusTransition = (currentStatus, newStatus) => {
  ...
  if (!allowed[currentStatus]?.includes(newStatus)) {
    throw createHttpError.BadRequest(`Cannot transition order from ${currentStatus} to ${newStatus}`);
  }
};
